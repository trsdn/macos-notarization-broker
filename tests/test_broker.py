...
class ProfileTests(unittest.TestCase):
    ...
    def test_profile_repository_identities_are_fixed(self) -> None:
        ...
        self.assertEqual(profiles["openconnect"]["repository_id"], 1341210851)
    ...
    def test_artifact_names_preserve_existing_release_contracts(self) -> None:
        ...
        self.assertEqual(
            names["openconnect"],
            [
                "OpenConnect-v{version}-macOS-universal.zip",
                "OpenConnect-v{version}-macOS-universal.dmg",
            ],
        )

    def test_profiles_shipping_nested_code_are_declared(self) -> None:
        # spacemender ships a privileged XPC helper; openconnect ships a CoreAudio
        ...
        self.assertEqual(shipping, {"openconnect", "spacemender"})
...
class PluginBundleValidationTests(BundleFixtureMixin, unittest.TestCase):
    ...
    This exercises the same shape as the openconnect profile: a two-architecture
...
# openconnect-make adapter uses it to drive a committed Makefile, and the
...
class BuildAdapterTests(unittest.TestCase):
    ...
    def test_openconnect_builds_the_committed_makefile(self) -> None:
        profile = broker.get_profile("openconnect")
        calls: list[list[str]] = []
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch.object(broker, "run", side_effect=fake_run):
                built = broker.build_openconnect(source, work, profile, "1.2.3", "42")
    ...
    def test_openconnect_build_requires_the_committed_makefile(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            source = Path(temporary) / "source"
            source.mkdir(parents=True)
            work = Path(temporary) / "work"
            ...
            with mock.patch.object(broker, "run", side_effect=AssertionError("must not build")):
                with self.assertRaises(broker.BrokerError):
                    broker.build_openconnect(
                        source, work, broker.get_profile("openconnect"), "1.2.3", "42"
                    )
