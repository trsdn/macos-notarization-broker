...
The `attest` job in `notarize.yml` makes GitHub sign a SLSA build-provenance
statement about the profile-selected notarized `.dmg` and `.zip` files with
this workflow's OIDC identity, so anyone can check that a downloaded file was
produced by this workflow at a given broker commit:

```bash
gh attestation verify OpenSwitchr-X.Y.Z.dmg --repo trsdn/macos-notarization-broker
```

OpenWritr is the one profile exception: the broker attests its ZIP only and
must never attest either DMG. Its updater alias is byte-identical to the
versioned DMG, so both names share one digest; attesting either name would make
an attestation discoverable for the updater download and can crash OpenWritr
1.6.0 while AppUpdater loads its Sigstore roots
([trsdn/OpenWritr#31](https://github.com/trsdn/OpenWritr/issues/31)). Both DMG
artifact declarations therefore set `attest: false`; the ZIP retains the
default `true`. A `copy_of` must share its source's policy because attestation
lookup is digest-based, not filename-based, and profile validation rejects any
profile with no attested artifact.
...
`contents: read`. It runs after `sign`, references no secret and no environment,
runs no shell, uses only two SHA-pinned actions, and fetches the notarized
artifact by the `sign` job's immutable artifact id, never by name, like every
other handoff here. It consumes only `attestation-subjects.sha256`, generated by
broker-owned signing code from final artifact bytes after notarization, stapling
and alias copying. It never sees the certificate, an Apple credential or the
source repository. Its OIDC token can only vouch for this workflow; it cannot
