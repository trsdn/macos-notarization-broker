...
def build_xcodegen_project(
    ...
    # These apps are generated by XcodeGen from a committed project.yml, and the generated
    # .xcodeproj is committed alongside it. The committed project is what gets built, so the
    # untrusted build job needs no XcodeGen and installs nothing; project.yml is pinned here only
    # so a checkout that lost it fails loudly instead of building something unintended.
    #
    # `directory` is "." for a project at the repository root.
    prefix = "" if directory in {"", "."} else f"{directory}/"
    ensure_source_file(source, f"{prefix}{project}/project.pbxproj")
    ensure_source_file(source, f"{prefix}project.yml")

    # A project depending on a Swift package by local path resolves to nothing if that path is
    # missing, and produces a bundle quietly missing whatever the package provided. Pin each such
    # path. A project depending on a package by URL and exact version does not need this, which is
    # why the list is per-caller rather than assumed.
    for package in local_packages:
        ensure_source_file(source, f"{package}/Package.swift")
...
def build_threemfquicklook(
    ...
    #
    # The project sits at the repository root, and its 3MF parsing comes from ThreeMFKit by URL at
    # an exact version rather than from a sibling directory, so there is no local package to pin.
    return build_xcodegen_project(
        source,
        work,
        profile,
